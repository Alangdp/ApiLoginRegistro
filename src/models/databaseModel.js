...
export default class Admin {
  constructor(databaseData = null) {
    let token = databaseData.DBToken;
    let actualPassword = databaseData.DBPassword || "null";
    ...
    this.data = {
      ...
      DBPassword: encrypt(actualPassword) || "null",
      DBPasswordBase: actualPassword || "null",
      DBNewPassword: newPassword,
      DBToken: token || genToken(),
      CreateBy: databaseData.CreateBy || "null"
    };
  ...
  async userExist(_id = null) {
    try {
      const userExist = await User.findById(_id || this.data.CreateBy);
  ...
  async showAll() {
    try {
      const teste = await DataBaseMongo.find({});
      console.log(13, teste)
      return teste
    } catch (err) {
      return {isOk: false, data: `code:${err.code} ${Object.keys(err.keyValue)}:${err.keyValue.CreateBy}`};
    }
